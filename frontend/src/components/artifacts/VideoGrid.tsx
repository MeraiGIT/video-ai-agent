...
export default function VideoGrid({ videos, totalScenes, onRegenerate }: Props) {
  const videoMap = new Map<number, string>();
  for (const vid of videos) {
    videoMap.set(vid.scene_index, vid.url);
  }
    ...
    <div className="animate-slide-up mx-2">
      <div className="flex items-center gap-2 px-1 mb-2">
        ...
        <span className="ml-auto text-xs text-gray-500">
          {videoMap.size} / {totalScenes}
        </span>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {Array.from({ length: totalScenes }, (_, i) => {
          const url = videoMap.get(i);
          return (
